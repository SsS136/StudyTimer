...
class StudyPageViewController: UIViewController {
    ...
    private let bottomTitle = ["New","Edit"]
...
//            UserDefaults.standard.register(defaults: ["Entire" : try! PropertyListEncoder().encode(Entire(DataSaver.subjects))])
        DataSaver.SaveEntireData(Entire(DataSaver.subjects))
        DataSaver.SaveMonthData(Month(monthBaseTime: 1000, monthCurrentTime: 10))
    }
    #endif
    private func setupBottomButton() {
        ...
        bottomButton.items = [
            ...
            FanMenuButton(id: "calender", image: UIImage(systemName: "calendar"), color: .firebrick),
            FanMenuButton(id: bottomTitle[1], image: UIImage(systemName: "pencil"), color:.darkKhaki)
        ]
